...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="xs xd txm" xpath-default-namespace="http://www.tei-c.org/ns/1.0"
    xmlns:txm="http://textometrie.org/1.0" xmlns:xd="http://www.pnp-software.com/XSLTdoc"
    version="2.0">
    
    <xd:doc type="stylesheet">
        <xd:short>
            This stylesheet "TEI-to-textlength-plot.xsl" serves to create a box plot 
            showing the length of texts in a corpus in number of tokens.
        </xd:short>
        <xd:detail>
            This stylesheet serves to create a box plot showing the length
            of texts in a corpus in number of tokens.
            ...
            XSLFile*: [PATH-TO-WORKING-DIRECTORY-ON-YOUR-COMPUTER]/TEI-to-textlength-plot.xsl
    ...
    <xsl:variable name="filename">textlength</xsl:variable>
    
    <xsl:template match="/">
        <!-- calculate this only once, for the first document in the corpus -->
        <xsl:variable name="id-this" select=".//text/@id"/>
        <xsl:variable name="corpus" select="collection($input-directory)//TEI"/>
        <xsl:variable name="id-first" select="$corpus[1]//text/@id"/>
                ...
                <html>
                    ...
                    <body>
                        ...
                        <script>
                            ...
                            var trace1 = {
                                y: [<xsl:for-each select="collection($input-directory)//TEI">
                                    <xsl:sort select="normalize-space(.//titleStmt/title)"/>
                            ...
                            var layout = {
                                ...
                                font: {size: 16},
                                title: {
                                    text: "Longitud de los textos en el corpus", // 'Length of the texts in the corpus'
                                    font: {size: 16}
                                },
                                xaxis: {
                                    font: {size: 22}
                                ...
                                yaxis: {
                                    title: "número de tokens", // "number of tokens"
                                    font: {size: 22}
                                }
                            };
